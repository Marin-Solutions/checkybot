<?php
    ...
    class ViewProjectsError extends Page implements HasInfolists
    {
        ...
        public function mount( int|string $record )
        {
            ...
            $this->errorStacktrace   = collect($error->stacktrace)
                ->map(fn( $item ) => ( $item[ 'class' ] ?? $item[ 'file' ] ) . " : {$item['line_number']}")
                ->all()
            ;
                        ...
                        ->tabs([
                            Tabs\Tab::make('Request')
                                ->schema([
                                    TextEntry::make('id')
                                        ->label('Method')
                                        ->formatStateUsing(fn( $state, $record ) => data_get($record, 'context.request.method', '-') . ' ' . data_get($record, 'context.request.url', '-'))
                                        ->badge()
                                        ->color('gray'),
