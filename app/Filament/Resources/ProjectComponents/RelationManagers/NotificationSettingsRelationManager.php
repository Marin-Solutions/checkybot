...
{
    private const COMPONENT_INSPECTION_OPTIONS = [
        WebsiteServicesEnum::APPLICATION_HEALTH->value => 'Application Health',
        WebsiteServicesEnum::ALL_CHECK->value => 'All Check',
    ];

    protected static string $relationship = 'notificationSettings';

    protected static ?string $title = 'Component Notifications';
        ...
        return $schema
            ->schema([
                Forms\Components\Select::make('inspection')
                    ->label('Monitor')
                    ->options(self::COMPONENT_INSPECTION_OPTIONS)
                    ->default(WebsiteServicesEnum::APPLICATION_HEALTH->value)
                    ->required(),
                ...
                Forms\Components\Select::make('notification_channel_id')
                    ...
                    ->options(fn (): array => $this->getOwnerRecord()->owner?->webhookChannels()?->pluck('title', 'id')->all() ?? [])
                    ->rules([
                        fn (Get $get): \Closure => function (string $attribute, $value, \Closure $fail) use ($get): void {
                            ...
                            $owner = $this->getOwnerRecord()->owner;
                            $owned = $owner !== null
                            ...
                            if (! $owned) {
                                $fail('Select one of this component owner\'s webhook channels.');
                            }
                        },
                    ])
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->emptyStateHeading('No component-specific alerts configured')
            ->emptyStateDescription('Add an alert below to route incidents from this application component to the right team channel.')
    ...
    protected function mutateNotificationData(array $data): array
    {
        ...
        $data['monitor_api_id'] = null;
        $data['project_component_id'] = $this->getOwnerRecord()->getKey();
        $data['scope'] = NotificationScopesEnum::PROJECT_COMPONENT->value;
