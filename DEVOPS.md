...
That script pulls from `origin master`, installs Composer dependencies from the committed lockfile, clears frontend dependencies before reinstalling them with `npm install --legacy-peer-deps`, builds frontend assets, caches framework metadata, runs migrations with `--force`, prunes Telescope data, and restarts queue/Horizon workers.
